<template>
  <div class="providers px-10 py-10">
    <h3>Proveedores</h3>
    <PxSearchProviders />
    <PxListItems :items="providers" />
  </div>
</template>
...
<script>
import PxSearchProviders from '@/pages/Providers/components/PxSearchProviders';
import PxListItems from '@/common-components/PxListItems';

export default {
  name: 'Providers',
  components: { PxListItems, PxSearchProviders },
  data: () => ({
  ...
  computed: {
    providers () {
      ...
      return Array.from({ length: 30 }, (k, v) => {
